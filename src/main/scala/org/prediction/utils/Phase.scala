...
import org.prediction.utils.Aggregator.BORDA
import org.prediction.utils.DBLPReader.globalGraph
...
case class Phase(startLearning: Int,
  ...
  def maximizationWeights(): Unit = {

    classifications.foreach {
      case (attribute, classification) =>
        attribute.maximizationOfPositivePrecision(positiveExamples,
                                                  classification)
    }
  }
  ...
  def saveInformation(): Unit = {
    ...
    graphPhase.triplets
      .repartition(1)
      .write
      .json("output/" + id + "/Learning/tripltes/")

    graphPhase.vertices
      .repartition(1)
      .write
      .json("output/" + id + "/Learning/vertices/")

    examples.repartition(1).write.json("output/" + id + "/examples/")

    labelingEdges.repartition(1).write.json("output/" + id + "/labelingEdges/")

    positiveExamples
      .repartition(1)
      .write
      .json("output/" + id + "/positiveExamples/")
    ...
    prediction.repartition(1).write.json("output/" + id + "/borda/")
  }
}
