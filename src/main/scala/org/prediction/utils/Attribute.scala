...
case class Attribute(
  ...
  /**
    ...
    */
  def maximizationOfPositivePrecision(
      positives: Dataset[Example],
    ...
    val firstNbPositives =
      classification.limit(5000).select($"_1".as[Example])
    val firtsNbPositivesInverse = firstNbPositives.map {
      case Example(c1, c2) => Example(c2, c1)
    }

    val truePositives = positives
      .intersect(firstNbPositives)
      .union(positives.intersect(firtsNbPositivesInverse))
      .count()

    scala.tools.nsc.io
      .File("/home/mohammedi/info.txt")
      .appendAll(
        s"for the attribute $id nbpos = $nbPositives, tp = $truePositives\n")

    val precision = truePositives / nbPositives
    history.put(1, weight)
    //weight = precision
  }

  def minimizationOfNegativePrecision(
      positives: Dataset[Example],
      classification: Dataset[(Example, Double)]): Unit = {
    ...
    val nbExmples = classification.count()
    ...
    val falsePositive = nbPositives - truePositives

    val nbNegatives = nbExmples - nbPositives
...
object Attribute {
  ...
  /**
    * Associer chaque nom d'attribut a son objet [[org.prediction.utils.Attribute]], cette [[scala.collection.Map]] contient tous
    * les attributs implémentés pour le moment (les attributs citées dans l'article de monsieur Kanawati)
    */
  val ALL_ATTRIBUTES = Seq(cn, jc, acc, pr)
